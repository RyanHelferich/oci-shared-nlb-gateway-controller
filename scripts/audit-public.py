...
PATTERNS = {
    ...
    "personal email address": re.compile(r"\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b", re.I),
    "North American phone number": re.compile(r"(?<!\d)(?:\+?1[-. ]?)?\(?[2-9]\d{2}\)?[-. ]\d{3}[-. ]\d{4}(?!\d)"),
...
def main():
    ...
    for path in text_files():
        ...
        for label, pattern in PATTERNS.items():
            if label in {"personal email address", "North American phone number", "US social security number"} and (relative.as_posix() == "THIRD_PARTY_NOTICES.json" or relative.as_posix().startswith("deploy/gateway-api-v1.6.2/")):
                continue
